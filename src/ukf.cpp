...
void UKF::initFilter(const MeasurementPackage &measurement) {
  if (measurement.sensor_type_ == MeasurementPackage::LASER) {
    // record the initial x and y position. we do not have velocity information.
    // So, assume 0
    x_ << measurement.raw_measurements_[0], measurement.raw_measurements_[1], 0,
        0, 0;
...
/**
 * Predict the mean and co-variance of the state from the predicted state points
 */
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  Tc.fill(0.0);
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  // residual
 ...
 */
void UKF::predictLidarSigmaPoints(VectorXd &z_pred, MatrixXd &S,
                                  MatrixXd &Zsig) {
 ...
 * @param Zsig - Predicted sigma points in measurement space
 */
void UKF::predictRadarSigmaPoints(VectorXd &z_pred, MatrixXd &S,
                                  MatrixXd &Zsig) {
  // transform sigma points into measurement space
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {
    double p_x = Xsig_pred_(0, i);
